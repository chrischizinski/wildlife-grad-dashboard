/**
 * Map Component
 * Renders a Leaflet map for job distribution
 */

window.renderJobMap = function (geographicData) {
    const mapContainer = document.getElementById('location-chart');
    if (!mapContainer) return;

    // Clear previous content (canvas or other)
    mapContainer.innerHTML = '<div id="leaflet-map" style="height: 100%; width: 100%; border-radius: 8px;"></div>';

    // Initialize Leaflet
    if (typeof L === 'undefined') {
        console.warn('Leaflet not loaded');
        mapContainer.innerHTML = '<div class="d-flex align-items-center justify-content-center h-100 text-muted">Map Unavailable</div>';
        return;
    }

    const map = L.map('leaflet-map').setView([37.8, -96], 4); // Center of US
    ...
    // Mock GeoJSON for US States (or load real one if available)
    // For this implementation, we will use a set of markers instead if no GeoJSON is available,
    // OR we can fetch a lightweight GeoJSON.

    // Let's use circle markers for states as a robust fallback without external heavy GeoJSON
    // Mapping of State names to coords
    const stateCoords = {
        ...
        // Mappings for generic regions if exact state unknown
        'Northeast US': [42.0, -74.0], 'Southeast US': [34.0, -84.0], 'Midwest US': [42.0, -90.0],
        'Southwest US': [34.0, -106.0], 'West US': [44.0, -118.0]
    };

    // Add markers
    Object.entries(geographicData).forEach(([location, count]) => {
        // Try exact match first
        let coords = stateCoords[location];

        // If not found, try to find a partial match in keys
        if (!coords) {
            const key = Object.keys(stateCoords).find(k => location.includes(k));
            if (key) coords = stateCoords[key];
        }

        if (coords) {
            L.circleMarker(coords, {
                radius: Math.max(6, Math.min(20, Math.sqrt(count) * 4)),
                fillColor: '#0ea5e9',
                color: '#fff',
                weight: 1,
                opacity: 1,
                ...
                .bindPopup(`<strong>${location}</strong><br>${count} Positions`)
                .addTo(map);
        }
    });
};
