...
class SupabasePopulator:
    ...
    def load_json_data(self) -> List[Dict[str, Any]]:
                ...
                try:
                    ...
                    if len(data) > 1:
                        ...
                        if len(unique_data) != len(data):
                            print(
                                f"Removed {len(data) - len(unique_data)} duplicate jobs"
                            )
        ...
        raise FileNotFoundError(
            "Could not find any job data files in expected locations"
        )
            ...
            try:
                ...
                for job in batch:
                    try:
                        ...
                        print(
                            f"Failed to insert job '{job.get('title', 'Unknown')}': {individual_error}"
                        )
    ...
    def verify_data(self):
        ...
        disciplines_result = (
            self.supabase.table("jobs")
            .select("discipline")
            .not_.is_("discipline", "null")
            .execute()
        )
        unique_disciplines = len(
            set(
                job["discipline"]
                for job in disciplines_result.data
                if job["discipline"]
            )
        )
        ...
        try:
            analytics_result = (
                self.supabase.table("job_analytics").select("*").execute()
            )
...
def main():
    ...
    try:
        ...
        if successful > 0:
            print("Database now contains updated job listings!")
        else:
